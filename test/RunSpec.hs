...
shiftCodeToEnd :: ProgramCode -> ProgramCode
shiftCodeToEnd c@(ProgramCode _ _ End) = c
...
spec = describe "Checking Run module with Word8 as a memory cell type" $ do
  context "run function" $ do
    it "'+' instruction" $ do
      let initialCode = parse $ VerifiedProgram "+"
          initialMem = emptyMemory @Word8
          initialPs = ProgramState initialMem initialCode
          finalPs = let p = change 1 initialPs in flip setCode p . shiftRCode $ programCode p
    ...
    it "'>' instruction" $ do
      let initialCode = parse $ VerifiedProgram ">"
          initialMem = emptyMemory @Word8
          initialPs = ProgramState initialMem initialCode
          finalPs = let p = move 1 initialPs in flip setCode p . shiftRCode $ programCode p
    ...
    prop "',' instruction" $ \i -> do
      let initialCode = parse $ VerifiedProgram ","
          initialMem = emptyMemory @Word8
          initialPs = ProgramState initialMem initialCode
          finalPs = let p = writeCell i initialPs in flip setCode p . shiftRCode $ programCode p
    ...
    it "'.' instruction" $ do
      let initialCode = parse $ VerifiedProgram "."
          initialMem = emptyMemory @Word8
          initialPs = ProgramState initialMem initialCode
          finalPs = flip setCode initialPs . shiftRCode $ programCode initialPs
  ...
  context "execProgram function" $ do
    prop "arbitrary number of '+' as a code"
      . forAll (choose (1, 99))
      $ \n -> do
        let initialCode = parse . VerifiedProgram $ replicate n "+"
            initialMem = emptyMemory @Word8
            initialPs = ProgramState initialMem initialCode
            finalPs = let p = change n initialPs in flip setCode p . shiftCodeToEnd $ programCode p
